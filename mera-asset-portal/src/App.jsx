...
// Default banner config - overridden by admin panel values from site_config table
...
function HubPage({ province, go }) {
  ...
  return (
    <div style={{ maxWidth: 960, margin: '0 auto', padding: '36px 24px' }}>
      {banner.active && (
        <div style={{ background: banner.color || '#004B6C', borderRadius: 12, marginBottom: 28, overflow: 'hidden' }}>
          {banner.imageUrl && (
          ...
          <div style={{ padding: '16px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16 }}>
            ...
            {banner.cta && banner.ctaUrl && (
              <a href={banner.ctaUrl} target="_blank" rel="noopener noreferrer"
                style={{ flexShrink: 0, background: '#fff', color: banner.color || '#004B6C', fontSize: 12, fontWeight: 600, padding: '8px 16px', borderRadius: 8, textDecoration: 'none', whiteSpace: 'nowrap' }}>
                {banner.cta}
              </a>
            )}
          </div>
          ...
          return (
            <div key={b.id} onClick={() => go({ view: 'brand', brandId: b.id })}
              ...
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 14 }}>
                {b.logoUrl
                  ? <img src={b.logoUrl} alt={b.name} style={{ height: 40, maxWidth: 120, objectFit: 'contain' }} />
  ...
  return (
    <div style={{ maxWidth: 1060, margin: '0 auto', padding: '28px 24px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 24, paddingBottom: 20, borderBottom: '1px solid #e8e6e0' }}>
        {b.logoUrl
          ? <img src={b.logoUrl} alt={b.name} style={{ height: 48, maxWidth: 140, objectFit: 'contain' }} />
      ...
      {skus.map(s => {
        ...
        return (
          <div key={s.id} onClick={() => go({ view: 'sku', brandId: nav.brandId, skuId: s.id })}
            ...
            {myAssets.length === 0 && <div style={{ fontSize: 11, color: '#bbb', marginTop: 4 }}>No assets yet</div>}
          </div>
        )
      })}
    </div>
...
// Inventory page - shows all provinces
