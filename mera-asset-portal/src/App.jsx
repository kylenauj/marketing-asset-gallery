...
const assetType = id => ALL_ASSET_TYPES.find(t => t.id === id)

// ─── top-level App ───────────────────────────────────────────────────────────
export default function App() {
    const [province, setProvince] = useState(null)
    // nav stack: { view: 'hub'|'brand'|'sku'|'type', brandId, skuId, typeId }
  const [nav, setNav] = useState({ view: 'hub' })

  const go = useCallback((next) => setNav(next), [])

  if (!province) return <ProvinceGate onSelect={p => { setProvince(p); setNav({ view: 'hub' }) }} />
  ...
  return (
        <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', background: '#f5f4f0' }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  {nav.view === 'type'  && <TypePage   province={province} nav={nav} go={go} />}
                  {nav.view === 'inventory' && <InventoryPage province={province} />}
                </div>div>
        </div>div>
      )
}
    ...
    return (
          <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg,#f5f4f0 0%,#e8f0e4 100%)' }}>
                  <div style={{ textAlign: 'center', maxWidth: 480, padding: '0 24px' }}>
                            <div style={{ width: 56, height: 56, background: '#004B6C', borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 24px', fontSize: 22, fontWeight: 500, color: '#fff' }}>M</div>div>
                            <h1 style={{ fontSize: 26, fontWeight: 500, color: '#1a1a1a', marginBottom: 8 }}>Retailer asset hub</h1>h1>
                            <p style={{ fontSize: 14, color: '#666', marginBottom: 36, lineHeight: 1.6 }}>Marketing materials for licensed Canadian retailers.<br />Select your province to get started.</p>p>
                        ...
                        <button key={p.code} onClick={() => onSelect(p)} style={{ padding: '18px 16px', border: '1px solid #e0deda', borderRadius: 12, background: '#fff', cursor: 'pointer', textAlign: 'center', fontFamily: 'var(--font)', transition: 'all 0.15s' }}
                                        ...
                                        <div style={{ fontSize: 22, fontWeight: 500, color: '#004B6C', marginBottom: 4 }}>{p.code}</div>div>
                                        <div style={{ fontSize: 13, color: '#555' }}>{p.name}</div>div>
                        </button>button>
                      ))}
                            </div>div>
                            <p style={{ marginTop: 20, fontSize: 12, color: '#999' }}>Don't see your province? <a href="mailto:hello@meracannabis.com" style={{ color: '#004B6C' }}>Contact us</a>a></p>p>
                  </div>div>
          </div>div>
        )
}
...
function Header({ province, nav, go, onSwitch }) {
    const crumbs = []
        if (nav.brandId) crumbs.push({ label: brand(nav.brandId)?.name || nav.brandId, action: () => go({ view: 'brand', brandId: nav.brandId }) })
    if (nav.view === 'sku' && nav.skuId) crumbs.push({ label: 'SKU Detail', action: null })
    if (nav.view === 'type' && nav.typeId) crumbs.push({ label: assetType(nav.typeId)?.label || nav.typeId, action: null })
  ...
  return (
        <div style={{ background: '#004B6C', padding: '14px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 10 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                          <div onClick={() => go({ view: 'hub' })} style={{ width: 34, height: 34, background: '#A2D074', borderRadius: 7, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, fontWeight: 500, color: '#2a4a0a', cursor: 'pointer' }}>M</div>div>
                          <span onClick={() => go({ view: 'hub' })} style={{ fontSize: 14, color: 'rgba(255,255,255,0.75)', cursor: 'pointer' }}>Hub</span>span>
                  {crumbs.map((c, i) => (
                    <span key={i} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                                  <i className="ti ti-chevron-right" style={{ fontSize: 11, color: 'rgba(255,255,255,0.35)' }} />
                                  <span onClick={c.action || undefined} style={{ fontSize: 14, color: c.action ? 'rgba(255,255,255,0.75)' : '#fff', fontWeight: c.action ? 400 : 500, cursor: c.action ? 'pointer' : 'default' }}>{c.label}</span>span>
                    </span>span>
                  ))}
                </div>div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                          ...
                          <span style={{ background: '#EDDC61', color: '#5a4e00', fontSize: 12, fontWeight: 500, padding: '4px 12px', borderRadius: 20 }}>{province.name}</span>span>
  ...
  return (
        <div style={{ maxWidth: 960, margin: '0 auto', padding: '36px 24px' }}>
                <div style={{ marginBottom: 28 }}>
                          <h2 style={{ fontSize: 22, fontWeight: 500, color: '#1a1a1a', marginBottom: 6 }}>Brands — {province.name}</h2>h2>
                          <p style={{ fontSize: 13, color: '#777' }}>Select a brand to browse its products and marketing assets.</p>p>
                </div>div>
                    ...
                    return (
                                  <div key={b.id} onClick={() => go({ view: 'brand', brandId: b.id })}
                                                  ...
                                                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 14 }}>
                                                    ...
                                                    {hasNew && <span style={{ background: '#d4f5d4', color: '#1a6b1a', fontSize: 10, fontWeight: 600, padding: '2px 8px', borderRadius: 20, letterSpacing: '0.04em' }}>NEW</span>span>}
                                                  </div>div>
                                                  <div style={{ fontSize: 17, fontWeight: 600, color: '#1a1a1a', marginBottom: 4 }}>{b.name}</div>div>
                                                  <div style={{ fontSize: 12, color: '#888' }}>
                                                    {count > 0 ? `${count} product${count !== 1 ? 's' : ''}${hasNew ? ` · ${newCounts[b.id]} new` : ''}` : 'No products yet'}
                                                  </div>div>
                                                  <div style={{ marginTop: 16, display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                                                    {SKU_ASSET_TYPES.map(t => (
                                                                      <span key={t.id} style={{ fontSize: 10, background: t.color, color: t.iconColor, padding: '2px 8px', borderRadius: 10, fontWeight: 500 }}>{t.label}</span>span>
                                                                    ))}
                                                  </div>div>
                                  </div>div>
                                )
...
// ─── Brand page — two views: SKU list + by asset type ────────────────────────
function BrandPage({ province, nav, go }) {
    const { brandId } = nav
    const b = brand(brandId)
    const [skus, setSkus] = useState([])
    const [brandAssets, setBrandAssets] = useState([])
    const [view, setView] = useState('skus') // 'skus' | asset-type id
  const [loading, setLoading] = useState(true)
    const [skuAssets, setSkuAssets] = useState([]) // all sku_assets for this brand+province
        ...
        <div style={{ maxWidth: 1060, margin: '0 auto', padding: '28px 24px' }}>
          {/* Brand header */}
                <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 24, paddingBottom: 20, borderBottom: '1px solid #e8e6e0' }}>
                          ...
                          <div>
                                    <h2 style={{ fontSize: 20, fontWeight: 600, color: '#1a1a1a', marginBottom: 2 }}>{b.name}</h2>h2>
                                    <div style={{ fontSize: 12, color: '#888' }}>{skus.length} product{skus.length !== 1 ? 's' : ''} in {province.name}</div>div>
                          </div>div>
                </div>div>
          ...
          {loading ? <Spinner /> : (
                  <>
                    {view === 'skus' && (
                                <SkuListView skus={skus} skuAssets={skuAssets} go={go} nav={nav} />
                              )}
                    {SKU_ASSET_TYPES.some(t => t.id === view) && (
                                <AssetTypeView typeId={view} skus={skus} skuAssets={skuAssets} go={go} nav={nav} />
                              )}
                    {BRAND_ASSET_TYPES.some(t => t.id === view) && (
                                <BrandAssetTypeView typeId={view} brandAssets={brandAssets} />
                              )}
                  </>>
                )}
        </div>div>
                                            ...
                                            return (
                                                        <div key={s.id} onClick={() => go({ view: 'sku', brandId: nav.brandId, skuId: s.id })}
                                                                    ...
                                                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                                                                                  <div style={{ fontSize: 14, fontWeight: 600, color: '#1a1a1a', flex: 1, marginRight: 8 }}>{s.name}</div>div>
                                                                      {s.is_new && <span style={{ background: '#d4f5d4', color: '#1a6b1a', fontSize: 10, fontWeight: 600, padding: '2px 7px', borderRadius: 20, whiteSpace: 'nowrap' }}>NEW</span>span>}
                                                                    </div>div>
                                                          {s.sku_code && <div style={{ fontSize: 11, color: '#999', marginBottom: 10 }}>SKU: {s.sku_code}</div>div>}
                                                                    <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
                                                                      {typeIds.map(tid => {
                                                                                        const t = SKU_ASSET_TYPES.find(x => x.id === tid)
                                                                                                          if (!t) return null
                                                                                                                            const cnt = myAssets.filter(a => a.asset_type === tid).length
                                                                                                                                              return <span key={tid} style={{ fontSize: 10, background: t.color, color: t.iconColor, padding: '2px 8px', borderRadius: 10, fontWeight: 500 }}>{t.label}{cnt > 1 ? ` (${cnt})` : ''}</span>span>
                                                                                                                                                })}
                                                                    </div>div>
                                                          {myAssets.length === 0 && <div style={{ fontSize: 11, color: '#bbb' }}>No assets yet</div>div>}
                                                        </div>div>
                                                      )
                })}
              </div>div>
...
// ─── Asset type view (e.g. all sell sheets for this brand) ────────────────────
                        ...
                        return (
                              <div>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                                            <div style={{ fontSize: 13, color: '#666' }}>{filtered.length} file{filtered.length !== 1 ? 's' : ''} across {[...new Set(filtered.map(a => a.sku_id))].length} product{[...new Set(filtered.map(a => a.sku_id))].length !== 1 ? 's' : ''}</div>div>
                          ...
                          // ─── Brand-level asset type view (logos, brandbooks, etc.) ───────────────────
                ...
                const [assets, setAssets] = useState([])
                    const [loading, setLoading] = useState(true)
                        ...
                        useEffect(() => {
                            ...
                            if (loading) return <div style={{ padding: 40 }}><Spinner /></div>div>
                                if (!sku) return null
                                ...
                                const grouped = {}
                                    assets.forEach(a => { if (!grouped[a.asset_type]) grouped[a.asset_type] = []; grouped[a.asset_type].push(a) })
                                        ...
                                        return (
                                                            ...
                                                            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
                                                                      <div>
                                                                                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                                                                                                ...
                                                                                                <span style={{ fontSize: 12, color: '#888' }}>{b?.name}</span>span>
                                                                                  </div>div>
                                                                                  <h2 style={{ fontSize: 20, fontWeight: 600, color: '#1a1a1a', marginBottom: 4 }}>{sku.name}</h2>h2>
                                                                        {sku.sku_code && <div style={{ fontSize: 12, color: '#999' }}>SKU: {sku.sku_code}</div>div>}
                                                                      </div>div>
                                                              {sku.is_new && <span style={{ background: '#d4f5d4', color: '#1a6b1a', fontSize: 11, fontWeight: 600, padding: '4px 10px', borderRadius: 20 }}>NEW</span>span>}
                                                            </div>div>
                                                    </div>div>
                                                                  ...
                                                                  return (
                                                                              <div key={t.id} style={{ marginBottom: 28 }}>
                                                                                          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
                                                                                                        ...
                                                                                                        <span style={{ fontSize: 14, fontWeight: 600, color: '#1a1a1a' }}>{t.label}</span>span>
                                                                                                        <span style={{ fontSize: 12, color: '#aaa' }}>({group.length})</span>span>
                                                                                            </div>div>
                                                                                {group.length === 0
                                                                                                ? <div style={{ fontSize: 12, color: '#bbb', padding: '10px 0' }}>No {t.label.toLowerCase()} uploaded yet</div>div>
                                          ...
                                          // ─── Type page (cross-brand view, kept for future) ────────────────────────────
function TypePage({ province, nav, go }) {
    return <div style={{ padding: 40, textAlign: 'center', color: '#888' }}>Coming soon</div>div>
      }
              ...
              <div style={{ background: '#fff', border: '1px solid #e8e6e0', borderRadius: 12, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
                    <div style={{ background: t.color || '#f5f4f0', padding: '14px 16px', display: 'flex', alignItems: 'center', gap: 10 }}>
                            ...
                            <div onClick={e => { e.stopPropagation(); onSkuClick && onSkuClick() }}
                              ...
                              {sku.name}{sku.is_new && <span style={{ marginLeft: 5, background: '#d4f5d4', color: '#1a6b1a', fontSize: 9, fontWeight: 700, padding: '1px 5px', borderRadius: 10 }}>NEW</span>span>}
                            </div>div>
                                      )}
                              {asset.file_size && <div style={{ fontSize: 10, color: '#aaa', marginTop: 1 }}>{asset.file_size}</div>div>}
                            </div>div>
                    </div>div>
    ...
    const [html, setHtml] = useState('')
        const [lastUpdated, setLastUpdated] = useState(null)
            const [loading, setLoading] = useState(true)
                ...
                useEffect(() => {
                      supabase.from('inventory').select('*').eq('province', province.code)
                              .order('updated_at', { ascending: false }).limit(1).maybeSingle()
                              .then(({ data }) => { setHtml(data?.html || ''); setLastUpdated(data?.updated_at || null); setLoading(false) })
                }, [province.code])
                    ...
                    return (
                          <div style={{ flex: 1, background: '#f5f4f0' }}>
                                <div style={{ maxWidth: 680, margin: '0 auto', padding: '28px 24px' }}>
                                        <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 20 }}>
                                                  <div style={{ fontSize: 18, fontWeight: 500, color: '#1a1a1a' }}>{province.name} — Weekly Inventory</div>div>
                                          {lastUpdated && <div style={{ fontSize: 12, color: '#999' }}>Updated {new Date(lastUpdated).toLocaleDateString()}</div>div>}
                                        </div>div>
                                  {loading && <Spinner />}
                                  {!loading && !html && <Empty icon="ti-package-off" text={`No inventory published yet for ${province.name}.`} />}
                                  {!loading && html && <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #e8e6e0', padding: 20 }} dangerouslySetInnerHTML={{ __html: html }} />}
                                </div>div>
                          </div>div>
                        )
                      }
                      ...
                      // ─── Shared UI ────────────────────────────────────────────────────────────────
function Spinner() {
    return <div style={{ textAlign: 'center', padding: '60px 0', color: '#999' }}><i className="ti ti-loader" style={{ fontSize: 24, display: 'block', marginBottom: 8, animation: 'spin 1s linear infinite' }} />Loading…</div>div>
      }
      function Empty({ icon, text }) {
          return <div style={{ textAlign: 'center', padding: '60px 0', color: '#bbb' }}><i className={`ti ${icon}`} style={{ fontSize: 28, display: 'block', marginBottom: 10 }} /><div style={{ fontSize: 13 }}>{text}</div>div></div>div>
            }</></div>
